    ...
    fetch("constant/content.json")
      ...
      .then((data) => {
        ...
        data.personalInfo.forEach((personalData) => {
          personalInfoList.appendChild(crearInformacionPersonal(personalData.label,personalData.data));
        })
        ...
        data.educacion.titulos.forEach((educacionData) => {
          educacionContenedor.appendChild(crearSeccionEducacion(educacionData.profesion,educacionData.fecha,educacionData.institucion,educacionData.descripcion));
        })
        ...
        data.experiencia.experienciaLaboral.forEach((personalExp) => {
          experienciaContenedor.appendChild(agregarExperiencia(personalExp.posicion,personalExp.fecha,personalExp.lugar,personalExp.actividades));
        })
        ...
        data.portafolio.filtros.forEach((filtro) =>{
          portafolioListaFiltros.appendChild(crearElementoFiltro(filtro.nombre,filtro.tipoFiltro));
        })
        let contador = 0;
        let ready = false;
        data.portafolio.proyectos.forEach((proyecto) => {
          portafolioContenido.appendChild(crearElementoPortafolio(proyecto.imagen,proyecto.pagina,proyecto.codigoFuente,proyecto.claseFiltro,contador++));
...
const crearElementoPortafolio = (imgPath, urlPage, urlFont, claseFiltro, numProyecto) => {
